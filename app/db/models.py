...
class Video(Base):
    ...
    metadata_status: Mapped[str] = mapped_column(String(16), default="pending", nullable=False)
    metadata_retry_count: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    metadata_next_retry_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    metadata_last_error: Mapped[str | None] = mapped_column(Text, nullable=True)
    metadata_fetched_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    metadata_tags: Mapped[str | None] = mapped_column(Text, nullable=True)
    metadata_clean_description: Mapped[str | None] = mapped_column(Text, nullable=True)
    metadata_hashtags: Mapped[str | None] = mapped_column(Text, nullable=True)
    metadata_sponsors: Mapped[str | None] = mapped_column(Text, nullable=True)
    metadata_urls: Mapped[str | None] = mapped_column(Text, nullable=True)
